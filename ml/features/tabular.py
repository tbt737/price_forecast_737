...
def feature_row(logy: np.ndarray, doy: np.ndarray, i: int) -> np.ndarray:
...
def training_matrix(
    logy: np.ndarray, doy: np.ndarray, *, horizon: int, end: int | None = None
    ...
    for i in range(LOOKBACK, stop):
        rows.append(feature_row(logy, doy, i))
