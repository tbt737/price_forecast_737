...
class StockReconcileConfig:
    ...
    min_reload_coverage: float = 1.0  # reloaded series must cover ALL stored dates — a
    # partial reload becomes the sole canonical series (single-basis read rule), so
    # anything below 1.0 lets a capped/paginated response silently drop stored dates
    # from every downstream read path forever (AUDIT-1B, confirmed HIGH)
