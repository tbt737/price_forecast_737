...
def _forecast_stub(pct: float, *, available: bool = True, model: str = "ridge_ar"):
    def stub(session, code, *, horizons):
        ...
        h = str(horizons[0])
        return {
            # Always "today" so this stub stays fresh against the real freshness
            # gate in main() no matter when the suite runs (was a fixed past date
            # that eventually aged past max_lag_trading_days and broke the test).
