...
- Guard chống reload cụt: `min_reload_coverage` (1.0) — series mới phải phủ **toàn bộ**
  số ngày đang stored, kèm 0 record invalid; vi phạm ⇒ `error`, không ghi gì (giữ nguyên
  basis cũ). Chặn việc một response bị cắt ngắn trở thành series canonical. (Trước đây
  ngưỡng 0.9 từng cho phép một reload thiếu tới 10% số ngày được chấp nhận làm basis mới
  — các ngày thiếu biến mất khỏi mọi đường đọc mà không có cảnh báo; xem AUDIT-1B.)
